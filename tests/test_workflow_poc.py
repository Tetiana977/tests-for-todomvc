...
def test_todos_management():
    ...
    ss('#todo-list>li').element_by(have.css_class('editing')).element('.edit').set_value('c edited').press_escape()

    ss('#todo-list>li').element_by(have.exact_text('c')).element('.destroy').click()
    ss('#todo-list>li').should(have.exact_texts('a', 'b'))

    ss('#todo-list>li').element_by(have.exact_text('a')).double_click()
    ss('#todo-list>li').element_by(have.css_class('editing')).element('.edit').set_value('a edited').press_enter()

    ss('#todo-list>li').element_by(have.exact_text('a edited')).element('.toggle').click()
